...
C NON-LINEAR (PIECEWISE LINEAR) HARDENING WITH DIFFERENT THRESHOLD IN
C PLANE STRESS:
C IMPLICIT PLANE STRESS-PROJECTED ELASTIC PREDICTOR/RETURN MAPPING
C ALGORITHM (BOXES 9.4-5).
C
C ALTERNATIVE REFERENCE: Section 9.4.3
C                        Boxes 9.4-5
C REFERENCE:   LINERO D. L. (2010)
C              OLIVER X., CERVERA M. (1990)
C***********************************************************************
...
C Compute the principal streses for cheking the hardening threshold
      ...
      DO 20 NPRINC=1,2
        ...
        ENDIF
   20 CONTINUE
      PFACT=(STPRIN(1)+STPRIN(2))/(ABS(STREST(1))+ABS(STREST(2)))
      PFACTT=PFACT*(1-(1/RPROPS(4)))+(1/RPROPS(4))
      PHI=PFACTT*RP5*XI-R1D3*SIGMAY*SIGMAY
...
C Compute new residual (yield function value)
          ...
          PHI=RP5*PFACTT*XI-R1D3*SIGMAY*SIGMAY
C         PHI=RP5*XI-R1D3*SIGMAY*SIGMAY
